...
func (sh *SeatHandler) findSeatByClass(c *gin.Context){
	...
	seatRequest := model.SeatRequestByClass{}
	c.ShouldBindJSON(&seatRequest)

	seats, err := sh.seatUsecase.FindSeatByClass(seatRequest)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{
			"message": "Internal server error",
			"error":   err.Error(),
...
func (sh *SeatHandler) deleteSeat(c *gin.Context){
	authToken := c.GetHeader("Authorization")
	...
	if role == "staff" {
		...
		deleteRequest := model.IDSeatRequest{}
		c.ShouldBindJSON(&deleteRequest)

		err = sh.seatUsecase.DeleteSeat(deleteRequest)
		if err != nil {
			c.JSON(http.StatusBadRequest, gin.H{
				"message": "Internal server error",
				"error":   err.Error(),
		...
		c.JSON(200, gin.H{
			"message": "Delete seat has been successful",
		})
	} else {
		c.JSON(http.StatusBadRequest, gin.H{
			"message": "User doesn't have permission to delete seat",
		})
	}
}
...
func (sh *SeatHandler) Route(r *gin.Engine) *gin.Engine{
	...
	public.DELETE("/deleteSeat", sh.deleteSeat)
